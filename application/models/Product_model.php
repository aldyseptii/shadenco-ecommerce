...
class Product_model extends CI_Model {
    ...
    private function getter() {
        $this->db->select("product.*,variant.id_variant,variant.name_variant,ukuran.id_ukuran,ukuran.name_ukuran,photo_product.id_photo,photo_product.url_photo,category.name_category,(SUM(comment.rating_comment) / COUNT(comment.name_comment)) AS total_rating");
        ...
        $this->db->join("ukuran", "ukuran.id_ukuran=product.id_product", "left");
    ...
    function get_product($id) {
        $this->db->select("product.*,photo.id_photo,photo.photo_product,category.name_category,motif.total_motif,size.ukuran_produk,    rating.total_rating", FALSE);
        $this->db->from("product");
        ...
        $this->db->join("(SELECT id_product,COUNT(variant.name_variant) AS total_motif FROM `variant` GROUP BY `id_product`) as motif", "motif.id_product=product.id_product", "left");
        $this->db->join("(SELECT id_product,COUNT(ukuran.name_ukuran) AS ukuran_produk FROM `ukuran` GROUP BY `id_product`) as size", "size.id_product=product.id_product", "left");
        $this->db->group_by("id_product");
