...
function gettotal(range1) {
    $.getJSON("<?=base_url();?>admin/report/total_json?range="+range1,function(result){
...
$('#tableReport').DataTable({
        ...
        "serverSide": true,
        "order": [],
        "ajax": {"url": "<?=base_url();?>admin/report/create_json?range="+range},
        "columnDefs": [
            {
                "targets": [ 3 ], 
                "className": "dt-right"
...
$('#reservation').on('change',function(){
    ...
    gettotal(range);
    $('#tableReport').DataTable().ajax.url("<?=base_url();?>admin/report/create_json?range="+range).load();
...
$('.btn-pdf').on("click",function(){
    ...
    location.href="<?=base_url("admin/report/pdf_report?range=");?>"+range;
});
</script>