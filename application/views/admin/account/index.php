...
$('#tableAccount').DataTable({
        ...
        "serverSide": true,
        "order": [],
        "ajax": {"url": "<?=base_url();?>admin/account/index_json"},
        "columnDefs": [
            { 
                "targets": [ 0 ], 
                "orderable": false, 
...
$('body').on('click','.btn-show',function(){
    ...
    if(action == "Edit") {
        ...
        $('#idedit').val(id);
        $.getJSON('<?=base_url('admin/account/get/');?>'+id,function(result){
...
$('.save').on('click',function(){
    ...
    if(action == 'Tambah') {
        var url = "<?=base_url("admin/account/add");?>";
    } else {
        var id = $('#idedit').val();
        var url = "<?=base_url('admin/account/edit/');?>"+id;
    }
...
$("body").on("click",".btn-delete",function(){
        ...
        Swal.fire({
          ...
          if (result.value) {
            $.getJSON("<?=base_url();?>admin/account/delete/"+id,function(response){
              if(!response['status']) {
                Swal.fire(
                    'Gagal',
                    response['msg'],
